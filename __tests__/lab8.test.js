describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    // (the first one has been checked)
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodcut_item = await page.$("product-item");

    // Grab the shadowRoot of that element (it's a property), then query a button from that shadowRoot.
    const shadow_root = await prodcut_item.getProperty ("shadowRoot");
    const button = await shadow_root.$("button");

    // Once you have the button, you can click it and check the innerText property of the button.
    await button.click ();
    const button_text = await button.getProperty ("innerText");

    // Once you have the innerText property, use innerText['_remoteObject'].value to get the text value of it
    const value = await button_text.jsonValue ();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    // (use page.$$() to select all of the elements)
    const prodcut_item = await page.$$("product-item");
    
    // (the first one has been added to cart)
    for (let i = 1; i < prodcut_item.length; i++) {
      // get the shadowRoot and query select the button inside, and click on it.
      const shadow_root = await prodcut_item[i].getProperty ("shadowRoot");
      const button = await shadow_root.$("button");
      await button.click ();
    }
    ...
    const button_text = await card_count.getProperty ("innerText");
    const value = await button_text.jsonValue ();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodcut_item.length; i++) {
      const shadow_root = await prodcut_item[i].getProperty ("shadowRoot");
      const button = await shadow_root.$("button");
      const button_text = await button.getProperty ("innerText");
      const value = await button_text.jsonValue ();
